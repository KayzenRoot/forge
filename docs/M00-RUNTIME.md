...
`NativeBoot` restores recorded token/cost consumption before issuing new resource leases. Callers must explicitly configure token/cost budgets; they default to zero. Live usage records require a stable 64-hex identity plus project, work-order, execution, capability and optional provider attribution. While the governor is healthy, an exact replay of a record already applied in memory is not charged twice, and conflicting reuse of an identity is rejected. Each live accounting operation holds a per-lease gate across validation, durable insertion and the in-memory charge, so a cloned lease cannot delegate while that transition is in flight. If cancellation, persistence ambiguity, a durable/live mismatch or a conflicting replay makes the in-memory state uncertain, the governor fails closed: new leases and lease activity, including replays, require recovery from a fresh `NativeBoot`, which replays the durable ledger. `ResourceError::AccountingInProgress` reports lease contention and `ResourceError::AccountingReconciliationRequired` reports the fail-closed state. The ledger fingerprint is included in the boot report.
