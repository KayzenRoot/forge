...
|---|---|---|---|
| S01 Kernel Runtime | `forge-kernel::boot`, `runtime` | runtime bound, native boot and cancellation tests | Candidate; platform-isolated CI pending |
...
| S21 Zero-Dependency Boot | `boot`, `forge-cli doctor` | Windows offline build and native smoke; Linux/network CI pending | Candidate; no platform PASS until blocked-network CI succeeds |
...
2. Complete exact-head Linux and Windows network/DNS-isolated `doctor` jobs and inspect their hosted results.
